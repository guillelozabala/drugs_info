...
def clean_total_reports_data(
    ...
    testservice_path = r'./data/processed/antenne_reports/testservice/'
    df.to_csv(testservice_path + 'total_samples_2023.csv', index=False)
...
def clean_dosering_data(
    ...
    for adjustment in settings.get('adjustments', []):
        ...
        if len(function_args) == 1:
            df[column] = df.apply(
                lambda row: function(row[column]),
                axis=1
            )
        else:
            df[column] = df.apply(
                lambda row: function(row[column], row.name),
                axis=1
            )

    # Column splitting
    split_columns = settings.get(
        'split_columns',
        {
            'Unnamed: 2': ['dose_min', 'dose_max'],
            column_map['dosering']: ['dose_mean', 'dose_sd']
        }
    )
    ...
    for col, new_cols in split_columns.items():
        df[new_cols] = (
            ...
            if '-' in df[col].iloc[0]
            else df[col].str.split(' ', expand=True)
        )

    # Get the price and number of samples priced columns
    price_col = settings.get(
        'price_column',
        column_map['prices_default']
    )

    n_price_col = settings.get(
        'n_price_column',
        column_map['n_prices_default']
    )
    ...
    df_filter = df.filter(
        settings.get('drop_columns', []) + [
            'Unnamed: 2', column_map['dosering'], 'prijs'
        ]
    )
    ...
    # Save processed data
    testservice_path = r'./data/processed/antenne_reports/testservice/'
    output_path = testservice_path + f'{substance}_dosering_2023.csv'
...
def clean_purity_data(
    ...
    if substance == 'mdma':
        missing_row_2023 = pd.Series(
            [2023, 35, 791, 97, 4, 0, 18, 2, 1, 0],
            index=df.columns
        )
    ...
    # Save processed data
    testservice_path = r'./data/processed/antenne_reports/testservice/'
    output_path = testservice_path + f'{substance}_purity_2023.csv'
    df.to_csv(output_path, index=False)
