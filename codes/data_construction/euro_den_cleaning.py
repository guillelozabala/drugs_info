...
df_filtered['MDMA_presentations'] = df_filtered['All presentations'] * df_filtered['MDMA (%)']/100
df_filtered['MDMA_and_NPS_presentations'] = df_filtered['MDMA_presentations'] + df_filtered['All presentations'] * df_filtered['Any NPS (%)']/100

yearly_sum_mdma = df_filtered.groupby('Year')['MDMA_presentations'].sum()

# Plot the results
plt.figure(figsize=(10, 6))
yearly_sum_mdma.plot(kind='bar')
plt.xlabel('Year')
plt.ylabel('Sum of Values')
plt.title('Sum of Values by Year (2013-2023)')
plt.show()

yearly_sum_mdma_nogermany = df_filtered[df_filtered['Country'] != 'Germany'].groupby('Year')['MDMA_presentations'].sum()
# Plot the results
plt.figure(figsize=(10, 6))
yearly_sum_mdma_nogermany.plot(kind='bar')
plt.xlabel('Year')
plt.ylabel('Sum of Values')
plt.title('Sum of Values by Year (2013-2023)')
plt.show()

yearly_sum_mdma_nps = df_filtered.groupby('Year')['MDMA_and_NPS_presentations'].sum()

# Plot the results
plt.figure(figsize=(10, 6))
yearly_sum_mdma_nps.plot(kind='bar')
plt.xlabel('Year')
plt.ylabel('Sum of Values')
plt.title('Sum of Values by Year (2013-2023)')
plt.show()